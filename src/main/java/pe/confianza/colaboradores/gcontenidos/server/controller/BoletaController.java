...
public class BoletaController {
    ...
    public void obtenerBoleta(HttpServletRequest httpServletRequest, HttpServletResponse response, @RequestBody RequestBoleta requestBoleta) {
        ...
        try {
			...
			SecurityUtils.validateUserSession(requestBoleta.getLogAuditoria().getUsuario());
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                Type founderListType = new TypeToken<List<BoletaModel>>() {
                }.getType();
    ...
    public ResponseEntity<?> obtenerBoletaString(HttpServletRequest httpServletRequest, HttpServletResponse response, @RequestBody RequestBoleta requestBoleta
        ...
        try {
            //Validamos usuario de la sesión
            if (requestBoleta.getLogAuditoria() != null) {
                if (requestBoleta.getLogAuditoria().getUsuario() != null) {
                    SecurityUtils.validateUserSession(requestBoleta.getLogAuditoria().getUsuario());
                } else {
                    responseStatus.setCodeStatus(99);
                    responseStatus.setMsgStatus("Auditoria: Usuario No Autorizado");
                    responseStatus.setResultObj("{}");
                    return new ResponseEntity<>(responseStatus, HttpStatus.UNAUTHORIZED);
                }
            } else {
                responseStatus.setCodeStatus(99);
                responseStatus.setMsgStatus("Es necesario los datos de auditoria");
                responseStatus.setResultObj("{}");
                return new ResponseEntity<>(responseStatus, HttpStatus.UNAUTHORIZED);
            }
