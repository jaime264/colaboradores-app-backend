...
public interface PublicacionDao extends MongoRepository<Publicacion, Long> {
	...
	public List<Publicacion> findAllUser(String user, Long lastPost, Pageable page);
	
	@Query(value="{'usuarios.usuarioBT': ?0, 'fecha': {$lt: ?1}}", sort="{'fecha': -1}", fields="{ '_id' : 0}")
	public List<Publicacion> findAllUserBack(String user, Long lastPost, Pageable page);
